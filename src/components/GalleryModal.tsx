...
export function GalleryModal({ project, onClose }: GalleryModalProps) {
  ...
  return (
    <motion.div
      ...
      onClick={onClose}
      className="fixed inset-0 z-[90] grid place-items-center bg-ink/95 p-2 sm:p-4 md:p-8"
      role="dialog"
      aria-modal="true"
    ...
    >
      <motion.div
        ...
        className="flex max-h-full w-full max-w-4xl flex-col border-4 border-paper bg-ink text-paper shadow-brutal-accent"
      >
        {/* Header */}
        <header className="flex items-center justify-between gap-3 border-b-4 border-paper px-3 py-2 sm:px-5 sm:py-3">
          <p className="min-w-0 truncate font-heading text-[11px] font-extrabold uppercase tracking-[0.2em] sm:text-sm">
            {project.title}
            <span className="ml-3 hidden font-mono text-[11px] font-normal tracking-widest text-paper/50 lg:inline">
              SCREENSHOT ARCHIVE
            </span>
          </p>
          <button
        ...
        {/* Viewer */}
        <div
          className="relative touch-pan-y border-b-4 border-paper bg-black"
        ...
        >
          <img
            ...
            draggable={false}
            className="mx-auto h-[36vh] w-full select-none object-contain object-center sm:h-[44vh] md:h-[52vh]"
          />
        ...
        {/* Filmstrip */}
        <div className="scrollbar-hide flex gap-1.5 overflow-x-auto p-2 sm:gap-2 sm:p-4">
          {project.screenshots.map((src, i) => (
            <button
              ...
              className={`relative h-12 w-[68px] shrink-0 cursor-pointer overflow-hidden border-[3px] p-0 transition-colors sm:h-14 sm:w-24 ${
            ...
            >
              <img
                ...
                alt=""
                loading="lazy"
                className={`h-full w-full object-cover ${i === index ? '' : 'grayscale hover:grayscale-0'}`}
              />
              <span className="absolute bottom-0 right-0 bg-ink/80 px-1 font-mono text-[8px] text-paper/80 sm:text-[9px]">
                {i + 1}
              </span>
            </button>
          ))}
